<script lang="ts">
  ...
  import Filter from "$components/Base/Table/Components/Filter.svelte";
...
<TableHeader
  title={"Abdullah"}
  subtitle={"View and manage your class with Abdullah efficiently."}
>
  <div
    slot="below-head"
  ...
  >
    ...
    <InputGroup class="w-full bg-white xs:max-w-sm">
      ...
      {#each courses.data as item}
        <TableRow class="items-center ">
          ...
          <TableCell class="col-span-3 text-sm font-normal text-[#475467]"
            >{item?.email ? item.email : "N/A"}</TableCell
          >
          ...
          <TableCell class="col-span-1 justify-end">

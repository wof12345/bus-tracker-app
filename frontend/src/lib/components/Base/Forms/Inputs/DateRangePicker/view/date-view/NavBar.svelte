<script>
  ...
  // import ArrowRight from '$lib/components/Assets/ArrowRight.svelte';
  // import ArrowLeft from '$lib/components/Assets/ArrowLeft.svelte';

  export let viewContextKey;

  const { months, config, highlighted, leftCalendarDate, rightCalendarDate } =
  ...
  $: canIncrementMonth =
    $monthView.monthIndex < months.length - 1 &&
    (config.isRangePicker && isStart ? myPosition < endPosition - 1 : true);
  $: canDecrementMonth =
    $monthView.monthIndex > 0 &&
    (config.isRangePicker && !isStart ? myPosition > startPosition + 1 : true);

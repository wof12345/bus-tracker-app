<script>
  ...
  function hasHiglightCondition(day, dates) {
    for (let date in dates) {
      let sessionDate = dayjs(date);

      if (dayjs(day).isSame(dayjs(sessionDate), "date")) {
        return true;
      }
    }
...
>
  {#each days as day}
    <div
      ...
      class:selection-end={config.isRangePicker &&
        day.date.isSame($selectedEndDate, "day")}
      class:part-of-range={config.isRangePicker &&
        isDateBetweenSelected($selectedStartDate, $selectedEndDate, day.date)}
      class:is-today={day.isToday}
      class:is-disabled={!day.selectable}
      class:is-highlighted={hasHiglightCondition(
        day.date,
        $datePickerStore.dates,
      )}
    >
      <button
        class="day--label"
        ...
        class:shake-date={$shouldShakeDate &&
          day.date.isSame($shouldShakeDate, "day")}
...
<style>
  ...
  .day.is-range-picker.selection-end .day--label:hover {
  ...
  .day.outside-month.selection-start.selection-end.is-today
    .day--label.highlighted,
  ...
  .day.is-range-picker.first-of-month:not(.outside-month).selection-end:not(
      .selection-start
    ):before,
  .day.is-range-picker.first-of-month:not(.outside-month).part-of-range {
    background: linear-gradient(
      to left,
      var(--passive-highlight-color) 70%,
      transparent
    );
  }
  .day.is-range-picker.last-of-month:not(.outside-month).selection-start:not(
      .selection-end
    ):before,
